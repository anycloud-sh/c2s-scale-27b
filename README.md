...
The image supports both `linux/amd64` (for H100 and B200 hosts) and
`linux/arm64` (for GH200 hosts). A single GPU with at least 64 GiB VRAM is
required; B200 is not required. If H100 capacity is unavailable, replace
`--gpu-type h100` with `--gpu-type gh200`.
...
docker buildx build \
  ...
  --provenance=false \
  -t ghcr.io/anycloud-sh/c2s-scale-27b:0.2.0-44c2ff7-r2 \
  .
```
